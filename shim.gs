...
chrome.i18n.getMessage = function(msg){
  return i18n[msg].message;
}

chrome.extension = {};
chrome.extension.sendMessage = function(obj){
...
chrome.storage.local.get = function(type){
  //console.log(options.get(type));
  return options.get(type)
};
chrome.storage.local.set = function(obj){
  var optionKey = Object.keys(obj).toString();
  var optionValue = obj[optionKey];
...
chrome.tabs = {};
chrome.tabs.create = function (obj){
  window.open(obj.url);
}
